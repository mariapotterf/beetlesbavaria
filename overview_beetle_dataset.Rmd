...
ips_data <- dat_cleaned_filtered %>% #dat_cleaned_sum_month %>%
  dplyr::filter(species == "Buchdrucker") %>%   ungroup(.)
...
ips_wide <- ips_wide %>%
  ...
  group_by(pairID) %>% 
  mutate(
    year_coverage = ifelse(all(final_year_range %in% year), "2018-2021", "else")
  ) #%>%
...
total number of trap pairs: 302, removed 
...
pairs_incomplete_years <- correlations_ips %>%
  dplyr::filter(year_coverage == "else") %>%
  pull(pairID) %>%
  unique()

print(pairs_incomplete_years)

# Calculate the number of unique pairIDs for each year coverage category
coverage_summary <- correlations_ips %>%
  group_by(year_coverage) %>%
  dplyr::summarize(unique_pairIDs = n_distinct(pairID))
...
summary_trap_pairs %>% 
  ggplot(aes(x = mean_cor ,
             ...
             #size = year_coverage,
             color = year_coverage)) +
  geom_point() + facet_grid(.~year_coverage)
