...
-   final: 136 trap pairs, removed were 23 trap (pairs) which actually were not pairs, but triplets, uncomplete..
...
```{r ips_high_dens_hist, include = T, warning = F, echo = F, fig.cap= "Distribution of share of 'fit classes' indicationg hig or low obeetle density over years. Dashed lines represent trends per fit-class and year. "}
...
ggplot(data = counts) +
  geom_bar(aes(x = as.factor(year), 
               y = share, 
               fill = beetle_density_cl), stat = "identity",alpha = 0.8, position = "dodge" ) +  # Use pre-calculated counts
...
summary_trap_ns <- correlations_ips %>%
  ...
  dplyr::summarize(
    ...
    years_no_fit = sum(beetle_density_cl == "no_fit", na.rm = TRUE), # Count years in "no_fit"
...
| beetle_density_cl   | Chr  | Categorization of counts based on the 3000 threshold: 'high' = both over 3000, 'no_fit' = one over 3000, where the one with the higher sgare is selected. 'lower' is excluded |
| class_percentage | Num  | Percentage of the specific `beetle_density_cl` class within all observations for the year. ('lower' is excluded)                                                                 |
...
| class_count      | Int  | Number of records falling into a specific `beetle_density_cl` category.               |
| class_percentage | Num  | Percentage of records in each `beetle_density_cl` category for a given year and pair. |
